...
const WANDER_TITLE_MAX_FLAT_EFFECT_VALUE = 200;
const WANDER_TITLE_MAX_RATIO_EFFECT_VALUE = 0.08;
...
const getWanderTitleEffectValueMax = (key: string): number => {
  return CHARACTER_RATIO_ATTR_KEY_SET.has(key) ? WANDER_TITLE_MAX_RATIO_EFFECT_VALUE : WANDER_TITLE_MAX_FLAT_EFFECT_VALUE;
};
...
const readWanderTitleEffectValue = (key: string, value: WanderAiJsonValue): number | null => {
  if (CHARACTER_RATIO_ATTR_KEY_SET.has(key)) {
    ...
    return normalized > 0 && normalized <= WANDER_TITLE_MAX_RATIO_EFFECT_VALUE ? normalized : null;
  }
...
const buildWanderTitleEffectEntrySchema = (): TechniqueTextModelJsonSchemaObject => {
  return {
    ...
    properties: {
      ...
      value: { type: 'number', exclusiveMinimum: 0, maximum: WANDER_TITLE_MAX_FLAT_EFFECT_VALUE },
    },
  };
};
