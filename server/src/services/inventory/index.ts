 ...
 *
 * 作用：整合 getInventoryItems 返回的物品实例与静态配置定义，
 *       为前端提供可直接展示的完整物品数据（含套装激活状态、基础属性折算、词条 roll 百分比等）。
 *       **不**修改数据库中的任何数据，纯只读聚合。
 *
 * 输入：
 * - characterId: 角色 ID
 * - location: 背包位置（bag / warehouse / equipped）
 * - page / pageSize: 分页参数
 *
 * 输出：
 * - items: 带 def 与 affixes 的物品数组
 * - total: 该位置的物品总数
 *
 * 数据流：
