 ...
 *
 * 作用（做什么 / 不做什么）：
 * - 做什么：统一计算“当前秘境实际要求的最低境界”，并校验所有参与者是否达到该门槛。
 ...
 * - 再批量读取参与者当前完整境界与昵称；
 * - 最后返回首个不满足门槛的参与者错误，供实例创建/加入/开战入口直接短路。
 *
 * 关键边界条件与坑点：
...
export const validateDungeonParticipantRealmAccess = async (params: {
  ...
  for (const participant of params.participants) {
    ...
    if (participantRank >= requiredRank) continue;

    const participantLabel = buildParticipantLabel(participant, participantNicknameMap);
    return {
      success: false,
      message: `${participantLabel}境界不足，需要达到${requiredRealm}`,
    };
  }
