...
test('buildWanderAiEpisodeSetupPromptRuleSet: 应显式包含幕次生成的主题、引子、标题与选项约束', () => {
  ...
  assert.equal(ruleSet.outputRules.optionTextLengthRange, '4-32');
  assert.equal(ruleSet.outputRules.openingLengthRange, '80-420');
  assert.match(ruleSet.outputRules.optionTextStyleRule, /options 必须是长度恰好为 3 的数组/u);
  assert.match(ruleSet.outputRules.optionResolutionRule, /非终幕推进/u);
...
test('buildWanderAiEpisodeSetupSystemMessage: 终幕待选幕次必须禁止提前写尾声与称号', () => {
  ...
  assert.match(systemMessage, /一次性生成待玩家选择的幕次，以及 3 条选项各自对应的结果草稿/u);
  assert.match(systemMessage, /游戏境界顺序示例/u);
  assert.match(systemMessage, /禁止写炼气期、筑基期、结丹期/u);
...
test('buildWanderAiEpisodeSetupUserPayload: 前文上下文与故事级伙伴和其他玩家快照必须完整透传', () => {
  ...
  assert.equal(payload.player.storyOtherPlayer?.realm, '炼炁化神');
  assert.equal(payload.player.storyOtherPlayer?.subRealm, '结胎期');
});
...
test('validateWanderAiEpisodeSetupContent: 合法待选幕次必须通过', () => {
  const validation = validateWanderAiEpisodeSetupContent(JSON.stringify({
    ...
    options: [
      {
        text: '先借檐避雨，再试探来意',
        summary: '你先借檐稳住桥上气机，再以灯影试探来客口风，逼得对方先露出几分破绽，却也让桥下暗潮顺势贴近了石栏。',
        endingType: 'none',
        rewardTitleName: '',
        rewardTitleDesc: '',
        rewardTitleColor: '',
      ...
      {
        text: '绕到桥下暗查灵息',
        summary: '你绕入桥下摸清暗潮来路，先一步看见潜伏在水雾里的异形轮廓，也因此把自己送进了更险的一线夹击里。',
        endingType: 'none',
        rewardTitleName: '',
        rewardTitleDesc: '',
        rewardTitleColor: '',
      ...
      {
        text: '收敛气机，静观其变',
        summary: '你先敛住气息静看桥上桥下两边动静，让来客与暗潮都误判了你的落点，局势因此被你拖进更深的试探。',
        endingType: 'none',
        rewardTitleName: '',
        rewardTitleDesc: '',
        rewardTitleColor: '',
  ...
  assert.equal(validation.data.options.length, 3);
  assert.equal(validation.data.options[0].text, '先借檐避雨，再试探来意');
});
...
test('buildWanderAiEpisodeResolutionUserPayload: 应透传本幕选择与终幕结算模式', () => {
  ...
  assert.equal(payload.player.storyOtherPlayer?.realm, '炼神返虚');
  assert.equal(payload.player.storyOtherPlayer?.subRealm, '养神期');
});
