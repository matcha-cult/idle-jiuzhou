...
test('buildWanderAiSystemMessage: 应在系统提示里强调短标题硬约束', () => {
  ...
  assert.match(systemMessage, /rewardTitleEffects 必须是长度 1 到 3 的数组/u);
...
test('buildWanderAiPromptRuleSet: 应显式暴露称号颜色与属性白名单约束', () => {
  ...
  assert.equal(ruleSet.outputRules.rewardTitleEffectCountRange, '1-3');
...
test('buildWanderAiResponseSchema: 可继续也可结局模式必须拆成非结局与结局两条互斥分支', () => {
  ...
  assert.equal(endingTitleEffectsSchema.maxItems, 3);
  ...
  assert.equal(endingTitleEffectsSchema.items.properties.value.type, 'number');
});
