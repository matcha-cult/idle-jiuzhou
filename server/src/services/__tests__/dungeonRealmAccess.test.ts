 ...
 * - 做什么：验证低于要求的参与者会被统一拦截，避免创建、加入、开战三个入口再次出现校验漂移。
...
test('validateDungeonParticipantRealmAccess: 应取秘境与难度中更高的境界要求', async (t) => {
  ...
  if (!result.success) {
    assert.equal(result.message, '队长【韩立】(角色ID:101)境界不足，需要达到炼精化炁·凝炁期');
  }
});

test('validateDungeonParticipantRealmAccess: 所有参与者达标时允许进入秘境', async (t) => {
  t.mock.method(participantHelpers, 'getParticipantNicknameMap', async () => {
    return new Map([
      [101, '韩立'],
      [102, '南宫婉'],
