 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1. 做什么：锁定伙伴招募、洞府研修与云游奇遇的“创建任务”入口必须先拿角色级事务互斥锁。
 * 2. 做什么：验证接入互斥锁后，这三条链路不再依赖 `FOR UPDATE` 读取最新任务/冷却行。
...
test('创建任务入口应复用角色级事务互斥锁并改用无锁状态读取', () => {
  ...
  const wanderSource = readFileSync(
  ...
  assert.match(wanderSource, /await lockWanderGenerationCreationMutex\(characterId\)/u);
  assert.match(wanderSource, /loadLatestGenerationJobRowByCharacterId\(characterId\)/u);
  assert.match(wanderSource, /loadLatestEpisodeRow\(characterId\)/u);
});
