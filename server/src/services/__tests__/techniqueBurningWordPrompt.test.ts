 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1. 做什么：验证用户指定提示词会统一走规范化、格式校验、敏感词校验与 prompt 语境构造。
 ...
 * 2. 敏感词检测必须复用现有共享服务，否则提示词会和角色名、功法命名形成不同词库口径。
 */
...
test('validateTechniqueBurningWordPrompt: 超过 2 个中文字符应被拦截', () => {
  ...
  assert.equal(result.message, '提示词最多 2 个中文字符');
});
...
test('validateTechniqueBurningWordPrompt: 非中文字符应被拦截', () => {
  ...
  assert.equal(result.success, false);
  if (result.success) return;
  assert.equal(result.message, '提示词只能包含中文字符');
});
...
test('buildTechniqueBurningWordPromptContext: 合法提示词应生成稳定 extraContext', () => {
