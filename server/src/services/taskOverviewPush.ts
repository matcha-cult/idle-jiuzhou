 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1. 做什么：把普通任务总览与悬赏总览的“脏通知”收敛为单一入口，避免路由、任务事件、悬赏写路径分别拼接同名 socket 事件。
 * 2. 做什么：只通知当前在线角色“哪些 overview 需要重拉”，让首页角标与任务弹窗继续复用现有 HTTP 快照接口，减少无关 `game:character` 刷新。
 * 3. 不做什么：不直接读取任务列表、不改任务状态，也不替代 `/task/overview` 与 `/task/bounty/overview` 的首屏快照职责。
 *
 * 输入/输出：
 ...
 *
 * 数据流/状态流：
 * 任务/悬赏写操作成功 -> 本模块去重并组装 scope payload -> `gameServer.emitToCharacter` -> 前端按 scope 决定刷新哪些 overview。
 *
 * 关键边界条件与坑点：
