 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1. 做什么：集中处理洞府研修“用户指定提示词”的规范化、格式校验、敏感词校验与 prompt 语境构造。
 * 2. 做什么：为路由、研修 service、后续脚本复用同一套文本规则，避免“最多 2 个字、只能中文、空白视为未填写”散落多处。
 * 3. 做什么：集中定义提示词只影响意象范围、不改变强度边界的提示词约束，供功法生成 prompt 单点复用。
 ...
 * 2. 提示词会直接进入模型提示词，因此格式校验之后仍必须经过敏感词检测，不能把违规字符带进任务记录和 prompt。
 * 3. 提示词只负责收束主题与文风，不能让模型误判为“可以突破品质与预算限制的特殊指令”，否则会产出明显超模的功法。
 */
import { guardSensitiveText } from '../sensitiveWordService.js';

const TECHNIQUE_BURNING_WORD_PROMPT_PATTERN = /^[\p{Script=Han}]+$/u;
const TECHNIQUE_BURNING_WORD_PROMPT_LABEL = '提示词';

export const TECHNIQUE_BURNING_WORD_PROMPT_MAX_LENGTH = 2;
export const TECHNIQUE_BURNING_WORD_PROMPT_SENSITIVE_MESSAGE = `${TECHNIQUE_BURNING_WORD_PROMPT_LABEL}包含敏感内容，请重新输入`;
...
export const TECHNIQUE_BURNING_WORD_PROMPT_GENERAL_RULE =
  `若 extraContext.techniqueBurningWordPrompt 存在，它表示玩家指定的${TECHNIQUE_BURNING_WORD_PROMPT_LABEL}；请围绕该${TECHNIQUE_BURNING_WORD_PROMPT_LABEL}延展功法命名、描述、技能意象与文风，但不要解释这个提示词，也不要把它输出成额外字段或固定前缀`;
export const TECHNIQUE_BURNING_WORD_PROMPT_SCOPE_GENERAL_RULE =
  `若 extraContext.techniqueBurningWordPromptScopeRules 存在，必须逐条遵守这些作用范围限制；${TECHNIQUE_BURNING_WORD_PROMPT_LABEL}只能收束主题与表现，不得借此突破品质、层数、效果数量、目标数量、倍率、冷却、被动预算等既有硬约束`;
export const TECHNIQUE_BURNING_WORD_PROMPT_SCOPE_RULES = [
  `${TECHNIQUE_BURNING_WORD_PROMPT_LABEL}只用于限定本次功法的主题意象、命名气质、描述文风、元素倾向与局部招式表现，不决定品质、层数、效果数量、目标数量与数值预算。`,
  `若${TECHNIQUE_BURNING_WORD_PROMPT_LABEL}与当前功法类型不完全贴合，应做同主题的合理化转译；可以保留更鲜明的核心套路与招式母题，但不要为了迎合${TECHNIQUE_BURNING_WORD_PROMPT_LABEL}强行拼接多体系、全覆盖或违和机制。`,
  `可以把${TECHNIQUE_BURNING_WORD_PROMPT_LABEL}延展成更鲜明、更偏锋的套路气质与招式表现，但不要生成全能通吃、超大范围、多段超高倍率、超长控制、超高回复或明显超出既有硬约束与预算的功法。`,
] as const;
...
export const validateTechniqueBurningWordPrompt = (
  ...
  if (getTechniqueBurningWordPromptLength(value) > TECHNIQUE_BURNING_WORD_PROMPT_MAX_LENGTH) {
    return {
      success: false,
      message: `${TECHNIQUE_BURNING_WORD_PROMPT_LABEL}最多 ${TECHNIQUE_BURNING_WORD_PROMPT_MAX_LENGTH} 个中文字符`,
  ...
  if (!TECHNIQUE_BURNING_WORD_PROMPT_PATTERN.test(value)) {
    return {
      success: false,
      message: `${TECHNIQUE_BURNING_WORD_PROMPT_LABEL}只能包含中文字符`,
      code: 'TECHNIQUE_BURNING_WORD_PROMPT_INVALID',
    };
  }
